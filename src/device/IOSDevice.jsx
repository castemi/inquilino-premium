/* Inquilino Premium — frame de dispositivo iOS (dynamic island + barra de estado + home indicator).
   El contenido (las pantallas) trae su propio header/tab bar, así que el frame solo aporta el chrome. */
...
export function IOSDevice({ children, width = 402, height = 874, dark = false, bare = false }) {
  // bare=true → sin bisel ni sombra (para móvil a pantalla completa)
  const frame = bare
    ? { width: '100%', height: '100%', borderRadius: 0, boxShadow: 'none' }
    : { width, height, borderRadius: 48, boxShadow: '0 40px 80px rgba(0,0,0,0.18), 0 0 0 1px rgba(0,0,0,0.12)' }
  return (
    <div style={{
      ...
      {/* barra de estado (absoluta, por encima del header de la pantalla) */}
